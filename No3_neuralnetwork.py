...
def init_network():
    ...
    pkl_path = os.path.join(base_dir, "deep-learning-from-scratch", "ch03", "sample_weight.pkl")
    if not os.path.exists(pkl_path):
        pkl_path = os.path.join(os.getcwd(), "deep-learning-from-scratch", "ch03", "sample_weight.pkl")
...
for i in range(0, len(x), batch_size):
    ...
    p = np.argmax(y_batch, axis=1) # axisとはy_batch.shape(100, 10)のどの次元でargmaxを取得するかの指定。今回は0ではなく1次元の方向に沿って最大を取得したいのでこの指定
